...
HTK（）是專門提來做語音辨識的開源工具，
伊頭仔先共`音檔`轉做`MFCC頻譜`，
才閣用`隱性馬可夫模型`、`高斯混合模型(Gaussion Mixture Model, GMM)`佮`決策樹(Decision Tree)`來訓練辨識模型，
...
用著`隱性馬可夫模型`、`高斯混合模型`佮`決策樹`來訓練`mgc頻譜`佮`lf0頻率對數`的預測模型，
尾仔才閣用`合成器（Vocoder）`共`mgc頻譜`佮`lf0頻率對數（log f0）`轉轉`音檔`。
