...
* decode
	* https://groups.google.com/forum/m/#!msg/kaldi-help/tAGblb-8Hy0/oMWnpgvhAAAJ
	* lm、fst
...
* lattice
  ...
  * 檢查聲學模型
    * https://groups.google.com/forum/m/#!msg/kaldi-help/HU5FT32EguU/cIMjnWRUAQAJ
* nnet3
  * https://groups.google.com/forum/m/#!msg/kaldi-help/BsJKFHpke9U/1lb9POsvAQAJ
* rnnlm
...
* script出現錯誤來除錯
	* data/lang/G.fst is not ilabel sorted
...
1. 用praat GUI程式，`minimun pitch`自100Hz調整到60Hz，`slicence  threshold`需要看音檔調整，其他會使用預設參數
...
呂道誠：華語之前Microsoft好像有用音素做，但近幾年大家都是用聲韻
...
##### 台文語料庫
1. 先將`通用拼音變調`轉成`臺羅拼音本調`
2. 仝一个語者，`連續劇`、`TW03`佮`EDU`資料夾內底仝名攏當做仝一个人。賰的仝一个檔內，仝名才當作仝人
3. 匯入`臺灣言語資料庫`

我手頭的語料庫毋是上新的款，而且我無`ALen&Tea`佮`Neighbor`的wav檔

##### 教育部閩南語常用詞辭典
[github程式](https://github.com/Taiwanese-Corpus/moedict-data-twblg/blob/%E5%85%88%E8%BD%89yaml%E6%A0%BC%E5%BC%8F/%E8%BD%89%E5%88%B0%E8%87%BA%E7%81%A3%E8%A8%80%E8%AA%9E%E8%B3%87%E6%96%99%E5%BA%AB/management/commands/%E5%8C%AF%E5%85%A5%E6%95%99%E5%85%B8%E9%9F%B3%E6%AA%94.py)
1. 共`詞目總檔`有音檔的詞條提出來
2. 語者攏標`王秀容`，毋過有16000Hz佮44100Hz的音檔，分幾仔个計劃錄的
3. 匯入`臺灣言語資料庫`

##### 唸教育部例句
1. 對`例句.csv`揣出7759句。標點符號結束的當作句，無標點符號的當作詞
2. 逐家到[網站](https://辨識.意傳.台灣)面頂，輸入家己的名，照例句編號錄
3. 照名匯入`臺灣言語資料庫`。其中高明達老師250句，丞宏293句，陳先生1000句

##### 語言模型
進前整理的[Taiwanese Corpus語料](https://github.com/sih4sing5hong5/tai5-uan5_gian5-gi2_hok8-bu7/wiki/Taiwanese-Corpus%E8%AA%9E%E6%96%99)，攏總73078句
1. [臺灣閩南語常用詞辭典](https://github.com/Taiwanese-Corpus/moedict-data-twblg/tree/gh-pages/%E8%BD%89%E5%88%B0%E8%87%BA%E7%81%A3%E8%A8%80%E8%AA%9E%E8%B3%87%E6%96%99%E5%BA%AB)
2. [臺灣閩南語卡拉OK正字字表](https://github.com/Taiwanese-Corpus/moe_minkalaok)
3. [iCorpus臺華平行新聞語料庫漢字臺羅版](https://github.com/Taiwanese-Corpus/icorpus_ka1_han3-ji7)
4. [咱的字你敢捌－台語漢字](https://github.com/Taiwanese-Corpus/Linya-Huang_2014_taiwanesecharacters)

閣有大條的資料庫可能會當先處理
1. [台語文數位典藏資料庫](https://github.com/Taiwanese-Corpus/nmtl_dadwt)
2. 台語文語料庫蒐集及語料庫為本台語書面語音節詞頻統計

##### 訓練過程
kaldi的eg[taiwanese](https://github.com/sih4sing5hong5/kaldi/tree/taiwanese/egs/taiwanese/s5c)，
自`swbd`改的

1. 先用`run.sh`訓練到`SAT（tri4)`佮clean_and_segment_data後的`tri5`
2. 閣用`清語料.sh`重做幾擺仔clean_and_segment_data，產生出`tri5.2`
