...
class 摩西翻譯模型訓練(腳本程式):
	def 訓練(self, 來源語言平行語料, 目標語言平行語料, 目標語言語料,
		...
		語言模型指令版 = \
			'{0}ngram-count -order {1} -interpolate -wbdiscount -unk {2} -lm {3}'
		語言模型指令 = 語言模型指令版.format(
			self.執行檔路徑加尾(SRILM執行檔路徑),
			連紲詞長度,
			' '.join(['-text ' + 語料 for 語料 in 目標語言語料]),
			語言模型檔
			)
		self.走指令(語言模型指令)
		...
		self._檔案合做一个(來源平行檔名, 來源語言平行語料)
		目標平行檔名 = os.path.join(暫存資料夾, '翻.目')
		self._檔案合做一个(目標平行檔名, 目標語言平行語料)
