...
class 轉物件音家私():
	# 逐个函式攏愛產生新的物件
	def _字轉做標準音標(self, 音標工具, 字物件, 標準函式):
		...
		if 字物件.型 in 標點符號 and 字物件.音 in 標點符號:
			...
			新音 = getattr(新音物件,標準函式)()
			if 新音 == None:
				raise 解析錯誤('音標無法度轉：{0}'.format(str(字物件)))
		else:
			新音 = 無音
		新型物件 = 音標工具(字物件.型)
		新型標準音標 = getattr(新型物件,標準函式)()
	...
	def _詞轉做標準音標(self, 音標工具, 詞物件, 標準函式):
		if not isinstance(詞物件, 詞):
			raise 型態錯誤('傳入來的毋是詞物件：{0}'.format(str(詞物件)))
		新詞物件 = 詞()
		for 字 in 詞物件.內底字:
			新詞物件.內底字.append(self._字轉做標準音標(音標工具, 字, 標準函式))
		return 新詞物件

	def _組轉做標準音標(self, 音標工具, 組物件, 標準函式):
		if not isinstance(組物件, 組):
			raise 型態錯誤('傳入來的毋是組物件：{0}'.format(str(組物件)))
		新組物件 = 組()
		for 詞 in 組物件.內底詞:
			新組物件.內底詞.append(self._詞轉做標準音標(音標工具, 詞, 標準函式))
		return 新組物件

	def _集轉做標準音標(self, 音標工具, 集物件, 標準函式):
		if not isinstance(集物件, 集):
			raise 型態錯誤('傳入來的毋是集物件：{0}'.format(str(集物件)))
		新集物件 = 集()
		for 組 in 集物件.內底組:
			新集物件.內底組.append(self._組轉做標準音標(音標工具, 組, 標準函式))
		return 新集物件

	def _句轉做標準音標(self, 音標工具, 句物件, 標準函式):
		if not isinstance(句物件, 句):
			raise 型態錯誤('傳入來的毋是句物件：{0}'.format(str(句物件)))
		新句物件 = 句()
		for 集 in 句物件.內底集:
			新句物件.內底集.append(self._集轉做標準音標(音標工具, 集, 標準函式))
		return 新句物件

	def _章轉做標準音標(self, 音標工具, 章物件, 標準函式):
		if not isinstance(章物件, 章):
			raise 型態錯誤('傳入來的毋是章物件：{0}'.format(str(章物件)))
		新章物件 = 章()
		for 句 in 章物件.內底句:
			新章物件.內底句.append(self._句轉做標準音標(音標工具, 句, 標準函式))
		return 新章物件

	def 轉做標準音標(self, 音標工具, 物件, 標準函式='標準音標'):
		if isinstance(物件, 字):
			return self._字轉做標準音標(音標工具, 物件, 標準函式)
		if isinstance(物件, 詞):
			return self._詞轉做標準音標(音標工具, 物件, 標準函式)
		if isinstance(物件, 組):
			return self._組轉做標準音標(音標工具, 物件, 標準函式)
		if isinstance(物件, 集):
			return self._集轉做標準音標(音標工具, 物件, 標準函式)
		if isinstance(物件, 句):
			return self._句轉做標準音標(音標工具, 物件, 標準函式)
		if isinstance(物件, 章):
			return self._章轉做標準音標(音標工具, 物件, 標準函式)
