...
class 集內組照排:
    def 排好(self, 排法, 物件):
        if isinstance(物件, 集):
            return self._排好集物件(排法, 物件)
        if isinstance(物件, 句):
            return self._排好句物件(排法, 物件)
        if isinstance(物件, 章):
            return self._排好章物件(排法, 物件)
        raise 型態錯誤('傳入來的毋是集句章其中一種物件：{0}，{1}'
                   .format(type(物件), str(物件)))

    def _排好集物件(self, 排法, 集物件):
