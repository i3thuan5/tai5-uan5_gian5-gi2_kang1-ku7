...
class 物件譀鏡:
	...
	def 看分詞(self, 物件, 物件分型音符號=分型音符號, 物件分字符號=分字符號, 物件分詞符號=分詞符號):
		if isinstance(物件, 字):
			return self.看字物件分詞(物件, 物件分型音符號, 物件分字符號, 物件分詞符號)
		if isinstance(物件, 詞):
			return self.看詞物件分詞(物件, 物件分型音符號, 物件分字符號, 物件分詞符號)
		if isinstance(物件, 組):
			return self.看組物件分詞(物件, 物件分型音符號, 物件分字符號, 物件分詞符號)
		if isinstance(物件, 集):
			return self.看集物件分詞(物件, 物件分型音符號, 物件分字符號, 物件分詞符號)
		if isinstance(物件, 句):
			return self.看句物件分詞(物件, 物件分型音符號, 物件分字符號, 物件分詞符號)
		if isinstance(物件, 章):
			return self.看章物件分詞(物件, 物件分型音符號, 物件分字符號, 物件分詞符號)
		self._掠漏.毋是字詞組集句章的毋著(物件)
	...
	def 看字物件分詞(self, 字物件, 物件分型音符號=分型音符號, 物件分字符號=分字符號, 物件分詞符號=分詞符號):
		self._掠漏.毋是字物件就毋著(字物件)
		if 字物件.音 == 無音:
	...
	def 看詞物件分詞(self, 詞物件, 物件分型音符號=分型音符號, 物件分字符號=分字符號, 物件分詞符號=分詞符號):
		self._掠漏.毋是詞物件就毋著(詞物件)
		字的音 = self._提著詞物件的字物件音陣列(詞物件, 物件分字符號, 物件分詞符號)
		無音數量 = 0
		for 音 in 字的音:
	...
	def 看組物件分詞(self, 組物件, 物件分型音符號=分型音符號, 物件分字符號=分字符號, 物件分詞符號=分詞符號):
		self._掠漏.毋是組物件就毋著(組物件)
		詞的音 = []
		for 一詞 in 組物件.內底詞:
			音標 = self.看詞物件分詞(一詞, 物件分型音符號, 物件分字符號, 物件分詞符號)
			if 音標 != '':
				詞的音.append(音標)
		return 物件分詞符號.join(詞的音)

	def 看集物件分詞(self, 集物件, 物件分型音符號=分型音符號, 物件分字符號=分字符號, 物件分詞符號=分詞符號):
		self._掠漏.毋是集物件就毋著(集物件)
		if len(集物件.內底組) == 0:
			raise 解析錯誤('內底組是空的！！')
		if len(集物件.內底組) > 1:
			raise 解析錯誤('內底組毋焦一个！！{0}'.format(str(集物件)))
		return self.看組物件分詞(集物件.內底組[0], 物件分型音符號, 物件分字符號, 物件分詞符號)

	def 看句物件分詞(self, 句物件, 物件分型音符號=分型音符號, 物件分字符號=分字符號, 物件分詞符號=分詞符號):
		self._掠漏.毋是句物件就毋著(句物件)
		集的音 = []
		for 一集 in 句物件.內底集:
			音標 = self.看集物件分詞(一集, 物件分型音符號, 物件分字符號, 物件分詞符號)
			if 音標 != 無音:
				集的音.append(音標)
		return 物件分詞符號.join(集的音)

	def 看章物件分詞(self, 章物件, 物件分型音符號=分型音符號, 物件分字符號=分字符號, 物件分詞符號=分詞符號):
		self._掠漏.毋是章物件就毋著(章物件)
		句的音 = []
		for 一句 in 章物件.內底句:
			音標 = self.看句物件分詞(一句, 物件分型音符號, 物件分字符號, 物件分詞符號)
			if 音標 != 無音:
				句的音.append(音標)
		return 物件分詞符號.join(句的音)
