...
from 資料庫.整合.教育部閩南語常用詞辭典 import 用流水號揣地區
...
# 			print(釋義編號)
			...
			for 例句, 標音, 例句翻譯 in 例句資料:
				...
				組字式型 = self.粗胚工具.符號邊仔加空白(例句).strip()
				標音 = self.粗胚工具.建立物件語句前處理減號(臺灣閩南語羅馬字拼音, 標音)
				標音 = self.粗胚工具.符號邊仔加空白(標音).strip()
