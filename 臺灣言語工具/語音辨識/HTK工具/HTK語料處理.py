...
class HTK語料處理(程式腳本):
    ...
    @classmethod
    def 處理試驗語料(cls, 音檔目錄, 資料目錄,
               標仔目錄=None, 音節聲韻對照檔=None, 執行檔路徑=外部程式.htk預設目錄()):
        全部語料 = cls.揣全部語料(音檔目錄, 標仔目錄)
        全部特徵檔 = os.path.join(資料目錄, '資料特徵檔.scp')
        os.makedirs(資料目錄, exist_ok=True)
        cls.揣特徵而且算(執行檔路徑, 資料目錄, 全部語料, 全部特徵檔)
        if 標仔目錄 is None:
            return 全部特徵檔
        ...
        cls.標仔收集起來(執行檔路徑, 全部標仔檔, 資料目錄, 用袂著的檔案, 音節檔)
        cls.標仔切做聲韻(執行檔路徑, 音節檔, 音節聲韻對照檔, 資料目錄, 聲韻類檔, 聲韻檔)
#         cls.標仔加恬佮切開(執行檔路徑, 全部標仔檔, 音節聲韻對照檔,
#             資料目錄, 音節檔, 聲韻類檔, 聲韻檔)
        return 全部特徵檔, 音節檔, 聲韻檔

    @classmethod
    def 揣特徵而且算(cls, 執行檔路徑, 資料目錄, 全部語料, 全部特徵檔):
        ...
        for 語料 in 全部語料:
            ...
            特徵所在 = os.path.join(特徵目錄,
                                語料名 + cls.特徵副檔名)
            cls.算特徵(執行檔路徑, 算特徵參數檔, 音檔所在, 特徵所在)
            全部特徵.append(特徵所在)
        cls._陣列寫入檔案(全部特徵檔, 全部特徵)

    @classmethod
    def 算特徵(cls, 執行檔路徑, 參數檔, 音檔所在, 特徵所在):
        特徵指令 = [
            join(執行檔路徑, 'HCopy'),
            '-A', '-C', 參數檔, 音檔所在, 特徵所在
        ]
    ...
    @classmethod
    def 標仔收集起來(cls, 執行檔路徑, 全部標仔檔, 資料目錄, 原來音類檔, 原來音節檔):
    ...
    @classmethod
    def 標仔切做聲韻(cls, 執行檔路徑, 音節檔, 音節聲韻對照檔, 資料目錄, 聲韻類檔, 聲韻檔):
        莫跳脫聲韻 = os.path.join(資料目錄, '莫跳脫聲韻.cfg')
        cls._字串寫入檔案(莫跳脫聲韻, 'noNumEscapes = T')
