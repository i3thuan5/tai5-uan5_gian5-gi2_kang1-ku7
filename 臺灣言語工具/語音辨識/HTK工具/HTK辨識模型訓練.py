...
class HTK辨識模型訓練(HTK語料處理):
    ...
    def 訓練原本標音辨識模型(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄(),
        ...
        (全部特徵檔, _全部標仔檔), (原始目錄, _原來音類檔, _原來音節檔, 原來聲韻類檔, 原來聲韻檔), 做好的初步模型檔 = cls._收集語料而且訓練原始音標初步模型(
            音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑
        )
        原來加混合模型 = cls.加混合數(執行檔路徑, 原始目錄, 全部特徵檔,
                           原來聲韻類檔, 原來聲韻檔, 做好的初步模型檔,
                           cls.孤音混合數, 估幾擺=10, 上尾估幾擺=50)
        return HTK辨識模型(
    ...
    def 訓練拄好短恬辨識模型(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄(),
        ...
        _新拄好短恬聲韻檔, _全部特徵檔, 加短恬辨識模型 = cls._訓練拄好短恬辨識模型閣回傳過程(
            音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑
        )
        return 加短恬辨識模型
    ...
    def 訓練三連音辨識模型(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄(),
        ...
        (全部特徵檔, 全部標仔檔), (_原始目錄, 原來音類檔, 原來音節檔, 原來聲韻類檔, _原來聲韻檔), 做好的初步模型檔 = cls._收集語料而且訓練原始音標初步模型(
            音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑)

        _短恬目錄, 拄好短恬的重估模型, 加恬聲韻類檔, 新拄好短恬聲韻檔 = cls._加短恬而且共時間傷短的短恬提掉(
            執行檔路徑, 資料目錄, 全部特徵檔, 全部標仔檔, 音節聲韻對照檔, 原來音類檔, 原來音節檔, 原來聲韻類檔, 做好的初步模型檔)

        三連音全部縛做伙模型, 三連音全部縛做伙聲韻類檔 = cls._三連音重估縛做伙加混合佮加無看過的音(
    ...
    @classmethod
    def 加短恬閣對齊(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄(),
               ):
        新拄好短恬聲韻檔, 全部特徵檔, 加短恬辨識模型 = cls._訓練拄好短恬辨識模型閣回傳過程(
            音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑
        )
    ...
    def _訓練拄好短恬辨識模型閣回傳過程(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄(),
        ...
        短恬目錄, 拄好短恬的重估模型, 加恬聲韻類檔, 新拄好短恬聲韻檔 = cls._加短恬而且共時間傷短的短恬提掉(
            執行檔路徑, 資料目錄, 全部特徵檔, 全部標仔檔, 音節聲韻對照檔, 原來音類檔, 原來音節檔, 原來聲韻類檔, 做好的初步模型檔)

        短恬加混合模型 = cls.加混合數(執行檔路徑, 短恬目錄, 全部特徵檔,
                           加恬聲韻類檔, 新拄好短恬聲韻檔, 拄好短恬的重估模型,
                           cls.孤音混合數, 估幾擺=10, 上尾估幾擺=50)

        return 新拄好短恬聲韻檔, 全部特徵檔, HTK辨識模型(
            音節聲韻對照檔=音節聲韻對照檔, 聲韻類檔=加恬聲韻類檔, 模型參數檔=短恬加混合模型
        )

    @classmethod
    def _收集語料而且訓練原始音標初步模型(cls,
                          音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑
                          ):
...
#         if 切標仔:
        ...
        cls.標仔收集起來(執行檔路徑, 全部標仔檔, 原始目錄, 原來音類檔, 原來音節檔)
        cls.標仔切做聲韻(執行檔路徑, 原來音節檔, 音節聲韻對照檔, 原始目錄, 原來聲韻類檔, 原來聲韻檔)
#         if 做初步模型:
        初步模型檔 = cls.建立初步模型(執行檔路徑, 原始目錄, 全部特徵檔, 原來聲韻類檔, 原來聲韻檔)
        做好的初步模型檔 = cls.模型重估(執行檔路徑, 原始目錄, 全部特徵檔, 原來聲韻類檔, 原來聲韻檔, 初步模型檔, 估幾擺=60)
#         上尾模型檔 = 做好的初步模型檔
#         上尾聲韻類檔 = 原來聲韻類檔
#         上尾聲韻檔 = 原來聲韻檔
        return (全部特徵檔, 全部標仔檔), (原始目錄, 原來音類檔, 原來音節檔, 原來聲韻類檔, 原來聲韻檔), 做好的初步模型檔
    ...
    def _加短恬而且共時間傷短的短恬提掉(cls,
        ...
        原本有短恬無 = cls.音類標仔加短恬(原來音類檔, 加恬音類檔)
        if 原本有短恬無:
            '有就免閣加矣'
            shutil.copy(原來音節檔, 加恬音節檔)
        elif len(cls._讀檔案(原來音類檔)) > 200:
            '音節轉聲韻，漢語聲韻加起來袂超過200'
            cls.音節標仔加短恬(原來音節檔, 加恬音節檔)
        else:
            '聲韻轉聲韻，漢語聲韻加起來袂超過200'
            cls.音節標仔逐字中央加短恬(原來音節檔, 加恬音節檔)
        cls.標仔切做聲韻(執行檔路徑, 加恬音節檔, 音節聲韻對照檔, 短恬目錄, 加恬聲韻類檔, 加恬聲韻檔)
        無短恬的模型 = 做好的初步模型檔
        if 原本有短恬無:
            無短恬的模型 = os.path.join(資料目錄, '提掉短恬的模型.macro')
            cls.模型提掉短恬(執行檔路徑, 短恬目錄,
                       做好的初步模型檔, 原來聲韻類檔, 無短恬的模型)
        加短恬的模型 = os.path.join(短恬目錄, '加短恬的模型.macro')
        cls.模型加短恬(無短恬的模型, 加短恬的模型)
        加短恬縛好模型 = os.path.join(短恬目錄, '加短恬縛好模型.macro')
        cls.恬音佮短恬愛縛做伙(執行檔路徑, 短恬目錄, 加短恬的模型, 加短恬縛好模型, 加恬聲韻類檔)
        加短恬的重估模型 = cls.模型重估(執行檔路徑, 短恬目錄, 全部特徵檔,
                            加恬聲韻類檔, 加恬聲韻檔, 加短恬縛好模型, 估幾擺=20)

        加恬辨識模型 = HTK辨識模型(音節聲韻對照檔=音節聲韻對照檔, 聲韻類檔=加恬聲韻類檔, 模型參數檔=加短恬的重估模型)
        對齊聲韻結果目錄 = 加恬辨識模型.對齊聲韻(
            加恬聲韻檔, 全部特徵檔, 短恬目錄, 執行檔路徑=執行檔路徑
        )
        新拄好短恬全部標仔檔 = os.path.join(短恬目錄, '新拄好短恬全部標仔檔.scp')
        cls._陣列寫入檔案(新拄好短恬全部標仔檔, cls.標仔換目錄(全部標仔檔, 對齊聲韻結果目錄))

        新對齊短恬聲韻檔 = os.path.join(短恬目錄, '新對齊短恬聲韻檔.mlf')
        用袂著的檔案 = os.path.join(短恬目錄, '用袂著的檔案.garbage')
        cls.標仔收集起來(執行檔路徑, 新拄好短恬全部標仔檔,
                   短恬目錄, 用袂著的檔案, 新對齊短恬聲韻檔)
        新拄好短恬聲韻檔 = os.path.join(短恬目錄, '新拄好短恬聲韻檔.mlf')
        cls.提掉傷短的短恬(新對齊短恬聲韻檔, 新拄好短恬聲韻檔)
        拄好短恬的重估模型 = cls.模型重估(執行檔路徑, 短恬目錄, 全部特徵檔,
                             加恬聲韻類檔, 新拄好短恬聲韻檔, 加短恬的重估模型, 估幾擺=60)
#         上尾模型檔 = 拄好短恬的重估模型
#         上尾聲韻類檔 = 加恬聲韻類檔
#         上尾聲韻檔 = 新拄好短恬聲韻檔
        return 短恬目錄, 拄好短恬的重估模型, 加恬聲韻類檔, 新拄好短恬聲韻檔
    ...
    def _三連音重估縛做伙加混合佮加無看過的音(cls, 執行檔路徑, 資料目錄,
        ...
        cls.音標換三連音(執行檔路徑, 三連音目錄, 新拄好短恬聲韻檔, 三連音聲韻類檔, 三連音聲韻檔)
        三連音模型檔 = os.path.join(三連音目錄, '三連音模型檔.macro')
        cls.模型換三連音(執行檔路徑, 三連音目錄, 拄好短恬的重估模型, 加恬聲韻類檔, 三連音模型檔, 三連音聲韻類檔)
        三連音重估模型 = cls.模型重估(執行檔路徑, 三連音目錄, 全部特徵檔,
...
#         上尾聲韻檔 = 三連音聲韻檔
    ...
    @classmethod
    def 建立初步模型(cls, 執行檔路徑, 資料目錄, 全部特徵檔, 聲韻類檔, 聲韻檔):
    ...
    @classmethod
    def 模型重估(cls, 執行檔路徑, 資料目錄, 全部特徵檔, 聲韻類檔, 聲韻檔, 原來模型檔, 估幾擺):
    ...
    @classmethod
    def 音節標仔加短恬(cls, 原來音節檔, 加恬音節檔):
        頂一逝, *後壁資料 = cls._讀檔案(原來音節檔)
        加短恬音節 = [頂一逝]
        for 一逝 in 後壁資料:
            if cls.是有音標仔(頂一逝) and cls.是有音標仔(一逝):
                加短恬音節.append(cls.短恬)
            加短恬音節.append(一逝)
            頂一逝 = 一逝
    ...
    @classmethod
    def 音節標仔逐字中央加短恬(cls, 原來音節檔, 加恬音節檔):
        cls.音節標仔加短恬(原來音節檔, 加恬音節檔)
        加短恬音節 = []
        目前短恬數量 = 0
    ...
    @classmethod
    def 提掉傷短的短恬(cls, 對齊加恬聲韻檔, 新拄好短恬聲韻檔):
        新聲韻 = []
...
'''	.format(cls.恬音, cls.短恬)
        cls._字串寫入檔案(恬音佮短恬愛縛做伙, 指令)
        縛做伙指令 = cls.改模型指令(
            執行檔路徑, 加短恬模型, 加短恬縛好模型, 恬音佮短恬愛縛做伙, 聲韻類檔)
        cls._走指令(縛做伙指令)

    @classmethod
    def 音標換三連音(cls, 執行檔路徑, 資料目錄,
...
""".format(三連音統計檔, 三連音聲韻類檔, 三連音縛做伙聲韻類檔, 三連音縛做伙樹檔)
        ...
        cls._字串寫入檔案(三連音縛做伙設定,
                    設定.format('\n'.join(問題設定), '\n'.join(縛做伙設定)))
        三連音縛做伙指令 = cls.改模型指令(執行檔路徑,
                             三連音模型檔, 三連音縛做伙模型, 三連音縛做伙設定, 三連音聲韻類檔)
        cls._走指令(三連音縛做伙指令)

    @classmethod
    def 模型加全部三連音(cls, 執行檔路徑, 資料目錄,
                 音節聲韻對照檔, 三連音縛做伙樹檔,
                 原來模型, 原來聲韻類檔, 原來縛做伙檔,
                 全部模型檔, 全部縛做伙檔):
...
'''
    ...
    @classmethod
    def 改模型指令(cls, 執行檔路徑, 頂一个模型, 下一个模型, 設定檔, 聲韻類檔):
        return [
            join(執行檔路徑, 'HHEd'), '-A',
