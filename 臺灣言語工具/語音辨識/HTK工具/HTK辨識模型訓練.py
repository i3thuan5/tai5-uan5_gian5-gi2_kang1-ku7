...
class HTK辨識模型訓練(HTK語料處理):
    ...
    def 訓練原本標音辨識模型(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄()):
        ...
        原來加混合模型 = cls._加混合數(執行檔路徑, 資料目錄, 全部特徵檔,
    ...
    def 訓練拄好短恬辨識模型(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄()):
        _全部特徵檔, _新拄好短恬聲韻檔, 加短恬辨識模型 = cls._訓練拄好短恬辨識模型閣回傳過程(
            音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑
        )
        return 加短恬辨識模型
    ...
    def 訓練三連音辨識模型(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄()):
        ...
        加恬聲韻類檔, 新拄好短恬聲韻檔, 拄好短恬的重估模型 = cls._加短恬而且共時間傷短的短恬提掉(
    ...
    def 加短恬閣對齊(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄()):
        全部特徵檔, 新拄好短恬聲韻檔, 加短恬辨識模型 = cls._訓練拄好短恬辨識模型閣回傳過程(
            音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑
        )
    ...
    def _訓練拄好短恬辨識模型閣回傳過程(cls, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄()):
        ...
        短恬加混合模型 = cls._加混合數(執行檔路徑, 資料目錄, 全部特徵檔,
                            加恬聲韻類檔, 新拄好短恬聲韻檔, 拄好短恬的重估模型,
                            cls.孤音混合數, 估幾擺=10, 上尾估幾擺=50)

        return 全部特徵檔, 新拄好短恬聲韻檔, HTK辨識模型(
...
#         if 做初步模型:
        ...
        做好的初步模型檔 = cls._模型重估(執行檔路徑, 原始目錄, 全部特徵檔, 原來聲韻類檔, 原來聲韻檔, 初步模型檔, 估幾擺=60)
#         上尾模型檔 = 做好的初步模型檔
#         上尾聲韻類檔 = 原來聲韻類檔
#         上尾聲韻檔 = 原來聲韻檔
    ...
    def _加短恬而且共時間傷短的短恬提掉(cls,
        ...
        原本有短恬無 = cls._音類標仔加短恬(原來音類檔, 加恬音類檔)
        if 原本有短恬無:
            '有就免閣加矣'
            shutil.copy(原來音節檔, 加恬音節檔)
        elif len(cls._讀檔案(原來音類檔)) > 200:
            '音節轉聲韻，漢語聲韻加起來袂超過200'
            cls._音節標仔加短恬(原來音節檔, 加恬音節檔)
        else:
            '聲韻轉聲韻，漢語聲韻加起來袂超過200'
            cls._音節標仔逐字中央加短恬(原來音節檔, 加恬音節檔)
        ...
        加短恬的重估模型 = cls._模型重估(執行檔路徑, 短恬目錄, 全部特徵檔,
        ...
        cls._陣列寫入檔案(新拄好短恬全部標仔檔, cls._標仔換目錄(全部標仔檔, 對齊聲韻結果目錄))
        ...
        cls._提掉傷短的短恬(新對齊短恬聲韻檔, 新拄好短恬聲韻檔)
        拄好短恬的重估模型 = cls._模型重估(執行檔路徑, 短恬目錄, 全部特徵檔,
                              加恬聲韻類檔, 新拄好短恬聲韻檔, 加短恬的重估模型, 估幾擺=60)
#         上尾模型檔 = 拄好短恬的重估模型
#         上尾聲韻類檔 = 加恬聲韻類檔
#         上尾聲韻檔 = 新拄好短恬聲韻檔
        return 加恬聲韻類檔, 新拄好短恬聲韻檔, 拄好短恬的重估模型,
    ...
    def _三連音重估縛做伙加混合佮加無看過的音(cls, 執行檔路徑, 資料目錄,
        ...
        三連音重估模型 = cls._模型重估(執行檔路徑, 三連音目錄, 全部特徵檔,
...
#         上尾聲韻類檔 = 三連音縛做伙聲韻類檔
#         上尾聲韻檔 = 三連音聲韻檔
        三連音加混合模型 = cls._加混合數(執行檔路徑, 三連音目錄, 全部特徵檔,
    ...
    def _音節標仔逐字中央加短恬(cls, 原來音節檔, 加恬音節檔):
        ...
        for 一逝 in cls._讀檔案(加恬音節檔):
            ...
            if 這逝_是有音標仔無:
                目前短恬數量 += 1
            if not 這逝_是有音標仔無 or 目前短恬數量 % 2 == 0:
                加短恬音節.append(一逝)
        cls._陣列寫入檔案(加恬音節檔, 加短恬音節)
