...
class HTK辨識模型(語料處理):
    ...
    def _對齊(self, 參數檔,  對照檔,
        ...
        對齊指令 = [
            join(執行檔路徑, 'HVite'), '-A',
            '-C', 參數檔, '-p', '-20', '-H', self.模型參數檔所在(), '-I', 標仔檔,
    ...
    def _辨識(self, 設定檔,  對照檔,  網路檔, 幾條網路, 特徵檔, 結果檔, 結果網路資料夾, 執行檔路徑=外部程式.htk預設目錄()):
        ...
        辨識指令 = [
            join(執行檔路徑, 'HVite'), '-A',
            '-C', 設定檔, '-p', '-20', '-H', self.模型參數檔所在(), '-w', 網路檔,
    ...
    def 辨識聲韻(self, 特徵檔, 資料目錄, 幾條網路, 執行檔路徑=外部程式.htk預設目錄()):
        ...
        self._生辨識網路(執行檔路徑, 資料目錄, self.聲韻類檔所在(),  網路檔)
    ...
    def 辨識音節(self, 特徵檔, 資料目錄, 幾條網路, 執行檔路徑=外部程式.htk預設目錄()):
        ...
        self._看聲韻類檔設定辨識音節的音節類檔(音節類檔)
        網路檔 = os.path.join(資料目錄, '音節網路檔.lat')
        self._生辨識網路(執行檔路徑, 資料目錄, 音節類檔, 網路檔)
    ...
    def _生辨識網路(self, 執行檔路徑, 資料目錄, 辨識類檔, 網路檔):
        辨識的可能 = set()
        短恬語法 = ''
        for 聲韻 in self._讀檔案(辨識類檔):
    ...
    def _看聲韻類檔設定辨識音節的音節類檔(self,  類檔):
        聲韻類 = set()
        for 聲韻 in self._讀檔案(self.聲韻類檔所在()):
    ...
    def _設定聲韻對照聲韻檔(self, 對照檔):
        對照表 = []
        for 類 in self._讀檔案(self.聲韻類檔所在()):
    ...
    def _設定指定參數檔(self, 上尾參數檔):
        ...
        if '-' in ''.join(self._讀檔案(self.聲韻類檔所在())):
