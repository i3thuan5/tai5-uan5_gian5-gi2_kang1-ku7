...
class HTK辨識模型(語料處理):
    ...
    def 對齊(self, 參數檔, 聲韻類檔, 對照檔, 模型檔,
        ...
        對齊指令 = [
            join(執行檔路徑, 'HVite'), '-A',
            '-C', 參數檔, '-p', '-20', '-H', 模型檔, '-I', 標仔檔,
            '-S', 特徵檔, '-o', 'S', '-y', 'lab', '-l', 結果夾, 對照檔, 聲韻類檔
        ]
        self._走指令(對齊指令)
        return

    def 對齊聲韻(self, 聲韻類檔, 模型檔, 聲韻檔, 特徵檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄()):
    ...
    def 對齊音節(self, 音節聲韻對照檔, 聲韻類檔, 模型檔, 音節檔, 特徵檔, 資料目錄, 執行檔路徑=外部程式.htk預設目錄()):
        參數檔 = os.path.join(資料目錄, '參數檔.cfg')
        self.設定參數(聲韻類檔, 參數檔)
    ...
    def 辨識(self, 設定檔, 聲韻類檔, 對照檔, 模型檔, 網路檔, 幾條網路, 特徵檔, 結果檔, 結果網路資料夾, 執行檔路徑=外部程式.htk預設目錄()):
        if int(幾條網路) > 0:
    ...
    def 辨識聲韻(self, 聲韻類檔, 模型檔, 特徵檔, 資料目錄, 幾條網路, 執行檔路徑=外部程式.htk預設目錄()):
        參數檔 = os.path.join(資料目錄, '參數檔.cfg')
        self.設定參數(聲韻類檔, 參數檔)
    ...
    def 辨識音節(self, 音節聲韻對照檔, 聲韻類檔, 模型檔,
             特徵檔, 資料目錄, 幾條網路, 執行檔路徑=外部程式.htk預設目錄()):
        參數檔 = os.path.join(資料目錄, '參數檔.cfg')
        self.設定參數(聲韻類檔, 參數檔)
