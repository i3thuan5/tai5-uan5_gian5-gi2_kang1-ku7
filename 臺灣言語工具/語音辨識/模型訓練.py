...
# 		if 加混合:
	...
	def 模型提掉短恬(self, 執行檔路徑, 資料目錄,
		...
		self.字串寫入檔案(提掉短恬的空設定, '')
		提掉短恬的聲韻類檔 = os.path.join(資料目錄, '提掉短恬的聲韻類檔.list')
		原來聲韻類 = set(self.讀檔案(原來有短恬的聲韻類檔))
		原來聲韻類.remove(self.短恬)
		self.陣列寫入檔案(提掉短恬的聲韻類檔, 原來聲韻類)
...
'''	.format(self.恬音, self.短恬)
	...
	def 音標換三連音(self, 執行檔路徑, 資料目錄,
		...
		聲韻類 = set(self.讀檔案(短恬相關三連音聲韻類檔))
		無恬相關聲韻類 = set(self._提掉恬的相關(聲韻類))
		無恬相關聲韻類.add(self.短恬)
		self.陣列寫入檔案(三連音聲韻類檔, 無恬相關聲韻類)
		
		self.陣列寫入檔案(三連音聲韻檔,
			self._提掉恬的相關(self.讀檔案(短恬相關三連音聲韻檔))
			)
