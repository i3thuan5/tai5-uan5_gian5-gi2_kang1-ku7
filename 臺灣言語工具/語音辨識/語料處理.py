...
class 語料處理(程式腳本):
    ...
    def 標仔收集起來(self, 執行檔路徑, 全部標仔檔, 資料目錄, 原來音類檔, 原來音節檔):
        ...
        self._字串寫入檔案(莫跳脫聲韻, 'noNumEscapes = T')
        整理音節指令 = [
            '{0}HLEd'.format(執行檔路徑), '-A', '-C', 莫跳脫聲韻, '-l', '*', '-n',
    ...
    def 標仔切做聲韻(self, 執行檔路徑, 音節檔, 音節聲韻對照檔, 資料目錄, 聲韻類檔, 聲韻檔):
        ...
        self._字串寫入檔案(切聲韻參數檔, 'EX')
        切聲韻指令 = ['{0}HLEd'.format(執行檔路徑), '-A', '-C', 莫跳脫聲韻, '-l', '*',
