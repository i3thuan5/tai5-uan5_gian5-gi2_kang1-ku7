...
from 臺灣言語工具.字詞組集句章.基本元素.公用變數 import 統一碼羅馬字類
from 臺灣言語工具.字詞組集句章.基本元素.公用變數 import 統一碼數字類
from 臺灣言語工具.字詞組集句章.基本元素.公用變數 import 聲調符號
...
# 					print(頭節 , 頭前漢字抑是組字式 , 頭前閣是組字式,後節 , 後壁漢字抑是組字式)
				...
				字元陣列.append(語句[位置])
			位置 += 1
		if 前回一開始狀態 == self.__一般 and 語句.endswith(分字符號):
			字元陣列.append(分詞符號)
