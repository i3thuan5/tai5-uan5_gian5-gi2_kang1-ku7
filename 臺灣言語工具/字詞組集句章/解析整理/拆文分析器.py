...
from 臺灣言語工具.字詞組集句章.基本元素.公用變數 import 斷句標點符號
...
class 拆文分析器:
	...
	def __init__(self):
		粗胚工具 = 文章粗胚工具()
		self.符號邊仔加空白 = 粗胚工具.符號邊仔加空白
		self.減號有照規則無 = 粗胚工具.減號有照規則無
				...
				if 字 in 分字符號:
					...
					if 語句[:位置].endswith(分詞符號) or 語句[位置 + 1:].startswith(分詞符號):
						...
						if len(佮後一个字是佇仝一个詞) == 0:
							if len(語句) > 1:
								raise 解析錯誤('一開始的減號是代表啥物？請用「文章粗胚工具.建立物件語句前處理減號」：語句{0}'.format(str(語句)))
							else:
								字陣列.append(字)
								佮後一个字是佇仝一个詞.append(False)
						else:
