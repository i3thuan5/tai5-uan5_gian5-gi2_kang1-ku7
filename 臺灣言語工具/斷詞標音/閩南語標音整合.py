...
from 臺灣言語工具.斷詞標音.資料庫揣辭典條目 import 資料庫揣辭典條目
...
class 閩南語標音整合:
	...
	def __init__(self, 腔口, 辭典):
		...
		for 流水號, 型體, 音標 in self.條目.揣腔口資料(腔口):
			處理過的音標 = self.粗胚工具.建立物件語句前處理減號(self.音標工具, 音標)
			# 愛加詞組無
			對齊組 = self.分析器.產生對齊組(型體, 處理過的音標)
	...
	def 語句斷詞標音(self, 語句):
		處理過的語句 = self.粗胚工具.建立物件語句前處理減號(self.音標工具, 語句)
		章物件 = self.分析器.建立章物件(處理過的語句)
		return self.物件斷詞標音(章物件)

	def 物件斷詞標音(self, 物件):
		轉教羅物件 = self.家私.轉做標準音標(self.教羅音標, 物件)
		正規物件 = self.家私.轉做標準音標(self.音標工具, 轉教羅物件)
		return self.斷詞標音.斷詞標音(self.辭典, 正規物件)
