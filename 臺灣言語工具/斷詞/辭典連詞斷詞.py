			...
			for 集物件 in 物件.內底集:
				...
				for 狀態, 分析 in 頂一層結果.items():
					...
					for 組物件 in 集物件.內底組:
						...
						結果 = self.字陣列斷詞(辭典, 連詞, 狀態, tuple(字陣列))
						for 這个狀態, 這个分析 in 結果.items():
							分數 , 這个頂一个狀態 , 這个頂一个詞 = 這个分析
							if 這个狀態 not in 集物件結果 or 分數 > 集物件結果[這个狀態]:
								集物件結果[這个狀態] = 狀態
# 							狀態
			上好分數 = None
			for 上尾狀態, 一个一个結果 in 結果.items():
				if len(上尾狀態[1]) == 0:
					分數= 一个一个結果[0]
...
# 					print(上尾狀態, 分數 , 答案詞陣列, )
	...
	def 字陣列斷詞(self, 辭典, 連詞, 頂一个狀態, 這馬字陣列):
		if hasattr(辭典, '空'):
