...
class 連詞揀集內組(TestCase):
	...
	def 揀(self, 連詞, 物件):
		...
		if isinstance(物件, 集):
			句物件 = 句()
			句物件.內底集.append(物件)
			標好句物件, 上好分數 , 詞數 = self._揀句物件音(連詞, 句物件)
			return (標好句物件.內底集[0], 上好分數, 詞數)
		if isinstance(物件, 句):
			return self._揀句物件音(連詞, 物件)
		if isinstance(物件, 章):
			return self._揀章物件音(連詞, 物件)
...
# 		return (物件, self.評分(連詞, 物件),)
	...
	def _揀章物件音(self, 連詞, 章物件):
		標好章物件 = 章()
		總分 = 0
		總詞數 = 0
		for 句物件 in 章物件.內底句:
# 			標好句物件, 分數 = self._揀句物件音(連詞, 句物件)
			標好句物件, 分數, 詞數 = self._揀句物件音(連詞, 句物件)
			標好章物件.內底句.append(標好句物件)
			總分 += 分數
			總詞數 += 詞數
		return (標好章物件, 總分, 總詞數)

	def _揀句物件音(self, 連詞, 句物件):
		開始組 = 組([連詞.開始])
...
# 			if 這馬組合!=None:
	...
	def _算上尾組物件分數(self, 連詞, 組陣列, 是毋是上尾一个):
		...
		return sum(評分結果) / len(評分結果), len(評分詞陣列) - len(頭前詞陣列)
