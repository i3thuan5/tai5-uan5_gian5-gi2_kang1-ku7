...
class 句物件轉合成標籤:
		...
		for 詞 in 臺羅音標.split(' '):
			...
			for 字 in 詞.split('-'):
				...
				if 拼音型態 == 臺灣閩南語羅馬字拼音:
					...
					聲, 韻 = (臺羅拼音.聲, 臺羅拼音.韻)
				if 聲 != '' and 聲 != None:
					聲標籤 = self.產生一般標仔(聲韻標籤[這馬第幾標籤 - 1], 聲韻標籤[這馬第幾標籤], 聲韻標籤[這馬第幾標籤 + 1],
						調標仔[句中第幾字 - 1], 調標仔[句中第幾字], 調標仔[句中第幾字 + 1],
						詞中第幾字, 詞攏總字數, 句中第幾字, 句攏總字數)
					這馬第幾標籤 += 1
					合成標籤.append(聲標籤)
				韻標籤 = self.產生一般標仔(聲韻標籤[這馬第幾標籤 - 1], 聲韻標籤[這馬第幾標籤], 聲韻標籤[這馬第幾標籤 + 1],
					調標仔[句中第幾字 - 1], 調標仔[句中第幾字], 調標仔[句中第幾字 + 1],
					詞中第幾字, 詞攏總字數, 句中第幾字, 句攏總字數)
				這馬第幾標籤 += 1
				if 字 == 'sil':
	...
	def 產生合成標仔(self, 前音, 此音, 後音, 前字調, 此字調, 後字調,
			詞中第幾字, 詞中上尾幾字, 詞攏總字數,
			句中第幾字, 句中上尾幾字, 句攏總字數):
		原本音標 = '{0}-{1}+{2}/tiau3:{3}<{4}>{5}/su5:{6}!{7}@{8}/ku3:{9}^{10}_{11}/giam7:'.format(
