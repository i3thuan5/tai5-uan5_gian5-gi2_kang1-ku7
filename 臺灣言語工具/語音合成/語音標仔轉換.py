...
class 語音標仔轉換:
	...
	_網仔 = 詞物件網仔()
	def __init__(self):
		self.恬標仔 = self._用全部資訊產生合成標仔(self.免知, self.恬音, self.免知,
	...
	def 物件轉完整合成標仔(self, 物件, 加短恬=False):
		if isinstance(物件, 章):
			return self._章物件轉完整合成標仔(物件, 加短恬)
		return self._句物件轉完整合成標仔(物件, 加短恬)
	...
	def _句物件轉完整合成標仔(self, 句物件, 加短恬):
		...
		合成標仔.append(self.恬標仔)
		return 合成標仔
	def _用全部資訊產生合成標仔(self, 前音, 此音, 後音, 前字調, 此字調, 後字調,
			詞中第幾字, 詞中上尾幾字, 詞攏總字數,
			句中第幾詞, 句中上尾幾詞, 句攏總詞數):
		原本音標 = self.音標格式.format(
			...
			句中第幾詞, 句中上尾幾詞, 句攏總詞數)
		return 原本音標
	def _用音節佮部份順序資訊產生合成標仔(self, 前音, 此音, 後音, 前字調, 此字調, 後字調,
			詞中第幾字, 詞攏總字數, 句中第幾詞, 句攏總詞數):
		return self._用全部資訊產生合成標仔(
			前音, 此音, 後音, 前字調, 此字調, 後字調,
