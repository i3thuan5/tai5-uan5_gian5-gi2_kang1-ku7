...
# 			總詞數 += 詞數
		...
		for 這馬集物件 in 句物件.內底集:
			...
			for 組合, 分數佮來源 in 全部分數佮來源[-1].items():
				...
				for 選擇組物件 in 這馬集物件.內底組:
					...
					這馬分數 = 分數 + self.算上尾組物件分數(連詞, 這馬組合)
					if 這馬組合 not in 這格分數佮來源:
						這格分數佮來源[這馬組合] = (這馬分數, 組合)
	...
	def 算上尾組物件分數(self, 連詞, 組陣列):
		頭前詞陣列 = []
		for 組物件 in 組陣列[:-1:-1]:
			頭前詞陣列.extend(組物件.內底詞)
			if len(頭前詞陣列) >= 連詞.上濟詞數:
				頭前詞陣列 = 頭前詞陣列[-連詞.上濟詞數:]
				break
		評分詞陣列 = 頭前詞陣列 + 組陣列[-1].內底詞
		return self.評詞陣列分(連詞, 評分詞陣列, len(頭前詞陣列))
