...
揣攏總資料 = 資料庫連線.prepare('SELECT "aid","year", ' +
	...
	'ORDER BY "aid" DESC LIMIT 2')

# 臺羅解析器 = 文章音標解析器(教會羅馬字音標)
# 臺羅解析器.合法字元 = {'-', ' '}
標題教羅解析器 = 文章音標解析器(何澤政教會羅馬字音標)
標題教羅解析器.標點符號 = {'-', ' '}
內文教羅解析器 = 文章音標解析器(何澤政教會羅馬字音標)
標點符號 = {' ', '-', ',', '。', '、', '，',
	'「','「', '」', '(', ')', '；', '？', '『', '』', '【', '】', '！', '：', '"'}
標點處理工具 = 文章標點處理工具()
標點處理工具.標點符號 = 標點符號
...
#	continue
	國語文章=[]
	音標文章=[]
	標題翻譯解析結果, 標題翻譯合法無 = 標題教羅解析器.解析語句佮顯示毋著字元(標題音標)
...
#			print(一个句語的詞)
	for 處理一半的內文音標 in 內文音標.split('\r\n'):
		for 一逝內文音標 in 處理一半的內文音標.split('\r\n'):
			...
			for 一个詞 in 一个句語的詞:
				內文翻譯解析結果, 內文翻譯合法無 = 內文教羅解析器.解析語句佮顯示毋著字元(一个詞,True)
