...
class 辨識模型:
	...
	def 對齊(self, 類檔, 對照檔, 模型檔, 標仔檔, 特徵檔, 結果檔, 執行檔路徑=''):
		執行檔路徑 = self.執行檔路徑加尾(執行檔路徑)
		對齊音指令 = '{0}HVite -A -p -20 -o N -H {1} -l "*" -t 400 -I {2} -S {3} -i {4} {5} {6}'\
			.format(執行檔路徑, 模型檔, 標仔檔, 特徵檔, 對齊音結果檔, 對照檔, 類檔)
		self.走指令(對齊音指令)
		return
	def 對齊聲韻(self, 聲韻類檔, 模型檔, 聲韻檔, 特徵檔, 資料目錄, 執行檔路徑=''):
		...
		return 對齊音結果檔
	def 對齊音(self, 音節聲韻對照檔, 模型檔, 音節檔, 特徵檔, 資料目錄, 執行檔路徑=''):
		音節類檔 = os.path.join(資料目錄, '音節類檔.list')
		self.家己類檔(音節聲韻對照檔, 音節類檔)
		對齊結果檔 = os.path.join(資料目錄, '對齊音結果檔.mlf')
		self.對齊(音節類檔, 音節聲韻對照檔, 模型檔, 音節檔, 特徵檔, 對齊結果檔, 執行檔路徑)
		return 對齊音結果檔
	...
	def 生辨識網路(self, 執行檔路徑, 資料目錄, 聲韻類檔, 網路檔):
		...
		self.走指令(產生網路指令)
	def 家己類檔(self, 對照檔, 類檔):
		對照表 = []
		for 類 in self.讀檔案(對照檔):
			對照表.append('{0}'.format(類.split()[0]))
		self.陣列寫入檔案(對照檔, 對照表)
...
if __name__ == '__main__':
	...
	對齊音結果檔 = 模型.對齊音(音節聲韻對照檔, 模型檔, 音節檔, 特徵檔, 資料目錄)
