...
class 辨識模型:
	...
	def 訓練(self, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑='',
		...
		if 加混合:
			...
			上尾模型檔 = 加混合了模型
		return 全部特徵檔, 加恬音節檔, 聲韻類檔, 聲韻檔, 上尾模型檔
	def 對齊(self, 聲韻類檔, 對照檔, 模型檔, 標仔檔, 特徵檔, 結果檔, 執行檔路徑=''):
		執行檔路徑 = self.執行檔路徑加尾(執行檔路徑)
		對齊指令 = '{0}HVite -A -p -20 -t 400 -H {1} -I {2} -S {3} -o N -l "*" -i {4} {5} {6}'\
			.format(執行檔路徑, 模型檔, 標仔檔, 特徵檔, 結果檔, 對照檔, 聲韻類檔)
		self.走指令(對齊指令)
		return
	def 對齊聲韻(self, 聲韻類檔, 模型檔, 聲韻檔, 特徵檔, 資料目錄, 執行檔路徑=''):
		聲韻對照檔 = os.path.join(資料目錄, '聲韻對照檔.dict')
		self.家己對照檔(聲韻類檔, 聲韻對照檔)
		對齊結果檔 = os.path.join(資料目錄, '對齊聲韻結果檔.mlf')
		self.對齊(聲韻類檔, 聲韻對照檔, 模型檔, 聲韻檔, 特徵檔, 對齊結果檔, 執行檔路徑)
		return 對齊結果檔
	def 對齊音節(self, 音節聲韻對照檔, 聲韻類檔, 模型檔, 音節檔, 特徵檔, 資料目錄, 執行檔路徑=''):
		對齊結果檔 = os.path.join(資料目錄, '對齊音節結果檔.mlf')
	...
	def 辨識(self, 聲韻類檔, 對照檔, 模型檔, 網路檔, 幾條網路, 特徵檔, 結果檔, 結果網路資料夾, 執行檔路徑=''):
		...
		if 幾條網路 > 0:
			...
			結果網路資料夾 = '*'
			幾條網路設定 = ''
		辨識指令 = '{0}HVite -A -p -20 -t 400 -H {1} -w {2} -S {3} -o N -y wav -z lattices -i {4} -l "{5}" {6} {7} {8}'\
...
if __name__ == '__main__':
	...
	特徵檔, 音節檔, 聲韻類檔, 聲韻檔, 模型檔 = 模型.訓練(
		這馬目錄 + '/wav', 這馬目錄 + '/labels', 音節聲韻對照檔, 資料目錄,
		算特徵=False)
	對齊聲韻結果檔 = 模型.對齊聲韻(聲韻類檔, 模型檔, 聲韻檔, 特徵檔, 資料目錄)
	對齊音節結果檔 = 模型.對齊音節(音節聲韻對照檔, 聲韻類檔, 模型檔, 音節檔, 特徵檔, 資料目錄)
	模型.辨識聲韻(聲韻類檔, 模型檔, 特徵檔, 資料目錄, 3,)
	模型.辨識音節(音節聲韻對照檔, 聲韻類檔, 模型檔, 特徵檔, 資料目錄, 3,)
