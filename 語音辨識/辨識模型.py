...
class 辨識模型:
	...
	def 訓練(self, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑='',
		...
		if 加切短恬:
			...
			加短恬的重估模型=self.模型重估(執行檔路徑, 資料目錄, 全部特徵檔, 加恬聲韻類檔, 加恬聲韻檔, 加短恬的模型, 估幾擺=60)
			對齊聲韻結果檔 = self.對齊聲韻(聲韻類檔, 加短恬的模型, 加恬聲韻檔, 全部特徵檔, 資料目錄, 執行檔路徑=執行檔路徑)
			上尾模型檔=
		return None, None
		if 加混合:
	...
	def 是有音標仔(self, 標仔):
		if 標仔 == '#!MLF!#' or 標仔 == self.音檔結束符號 or\
				標仔.startswith('"') or 標仔.startswith('sil/'):
			return False
		return True
	def 標仔切做聲韻(self, 執行檔路徑, 音節檔, 音節聲韻對照檔, 資料目錄, 聲韻類檔, 聲韻檔):
		...
		切聲韻指令 = '{0}HLEd -A -l "*" -i {1} -n {2} -d {3} {4} {5}'\
			.format(執行檔路徑, 聲韻檔, 聲韻類檔, 音節聲韻對照檔, 切聲韻參數檔, 音節檔)
		self.走指令(切聲韻指令)
