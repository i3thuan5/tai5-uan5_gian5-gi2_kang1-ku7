...
class 辨識模型:
	...
	def 訓練(self, 音檔目錄, 標仔目錄, 音節聲韻對照檔, 資料目錄, 執行檔路徑='',
		...
		if 加切短恬:
			...
			加短恬的重估模型 = self.模型重估(執行檔路徑, 資料目錄, 全部特徵檔,
				加恬聲韻類檔, 加恬聲韻檔, 加短恬的模型, 估幾擺=1)
			對齊聲韻結果檔 = self.對齊聲韻(加恬聲韻類檔, 加短恬的模型, 加恬聲韻檔, 全部特徵檔, 資料目錄, 執行檔路徑=執行檔路徑)
			新拄好短恬聲韻檔 = os.path.join(資料目錄, '新拄好短恬聲韻檔.mlf')
			self.提掉傷短的短恬(對齊聲韻結果檔, 新拄好短恬聲韻檔)
			拄好短恬的重估模型 = self.模型重估(執行檔路徑, 資料目錄, 全部特徵檔,
				加恬聲韻類檔, 加恬聲韻檔, 加短恬的重估模型, 估幾擺=1)
			上尾模型檔 = 拄好短恬的重估模型
		return None, None
		if 加混合:
	...
	def 對齊(self, 聲韻類檔, 對照檔, 模型檔, 標仔檔, 特徵檔, 結果夾, 執行檔路徑=''):
		...
		對齊指令 = '{0}HVite -A -p -20 -t 400 -H {1} -I {2} -S {3} -o S -y lab -l "{4}" {5} {6}'\
