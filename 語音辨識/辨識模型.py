...
# 		if 做初步模型:
		...
		if 加切短恬:
			...
			加短恬綁好模型 = os.path.join(資料目錄, '加短恬綁好模型.macro')
			self.恬音佮短恬愛綁做伙(執行檔路徑, 資料目錄, 加短恬的模型, 加短恬綁好模型, 加恬聲韻類檔)
			加短恬的重估模型 = self.模型重估(執行檔路徑, 資料目錄, 全部特徵檔,
				加恬聲韻類檔, 加恬聲韻檔, 加短恬綁好模型, 估幾擺=20)
...
# 			資料目錄, 音節檔, 聲韻類檔, 聲韻檔)
	...
	def 恬音佮短恬愛綁做伙(self, 執行檔路徑, 資料目錄,
			加短恬模型, 加短恬綁好模型, 聲韻類檔):
		執行檔路徑 = self.執行檔路徑加尾(執行檔路徑)
		恬音佮短恬愛綁做伙 = os.path.join(資料目錄, '恬音佮短恬愛綁做伙.hed')
		指令 = '''\
AT 2 4 0.2 {{{0}.transP}}
AT 4 2 0.2 {{{0}.transP}}
AT 1 3 0.3 {{{1}.transP}}
TI 短恬綁恬音 {{{0}.state[3],{1}.state[2]}}\
'''	.format(self.恬音, self.短恬)
		self.字串寫入檔案(恬音佮短恬愛綁做伙, 指令)
		加混合數指令 = '{0}HHEd -A -H {1} -w {2} {3} {4}'\
			.format(執行檔路徑, 加短恬模型, 加短恬綁好模型, 恬音佮短恬愛綁做伙, 聲韻類檔)
		self.走指令(加混合數指令)
