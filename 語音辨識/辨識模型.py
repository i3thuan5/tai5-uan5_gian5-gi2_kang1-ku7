...
# 		if 做初步模型:
		...
		if 加切短恬:
			...
			self.陣列寫入檔案(新拄好短恬全部標仔檔, self.揣全部標仔(對齊聲韻結果檔))
...
# 			資料目錄, 音節檔, 聲韻類檔, 聲韻檔)
	...
	def 揣全部標仔(self, 標仔目錄):
		全部標仔 = []
		for 標仔檔名 in os.listdir(標仔目錄):
			if 標仔檔名.endswith(self.標仔副檔名):
				標仔所在 = os.path.join(標仔目錄, 標仔檔名)
				全部標仔.append(標仔所在)
		return 全部標仔
