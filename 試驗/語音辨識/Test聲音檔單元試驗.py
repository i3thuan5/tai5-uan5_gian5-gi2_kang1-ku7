...
class 聲音檔單元試驗(TestCase):
    ...
    def test_無整數的音框數量愛著(self):
        音檔 = 聲音檔.對參數轉(
            2, 999, 2,
            b'01234567890123456789' * 200
        )
        全部資料 = []
        for 音框 in 音檔.全部音框(音框秒數=0.02):
            全部資料.extend(音框.wav音值資料())
        self.assertEqual(len(全部資料), 1000)

    def test_無整數的音框內容愛著(self):
        音檔 = 聲音檔.對參數轉(
            2, 999, 2,
            b'01234567890123456789' * 200
        )
