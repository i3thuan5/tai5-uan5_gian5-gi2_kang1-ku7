...
class 語音標仔轉換單元試驗(unittest.TestCase):
    ...
    def test_臺灣閩南語羅馬字拼音完整標仔(self):
        ...
        句物件 = 拆文分析器.產生對齊句(self.閩南語, self.閩南語)
    ...
    def test_臺灣閩南語羅馬字拼音逐字加短恬完整標仔(self):
        ...
        句物件 = 拆文分析器.產生對齊句(self.閩南語, self.閩南語)
        音值句物件 = 轉物件音家私.轉音(臺灣閩南語羅馬字拼音, 句物件, 函式='音值')
        完整標仔 = 語音標仔轉換.物件轉完整合成標仔(音值句物件, 加短恬=True)
        self.檢驗標仔有對無(完整標仔,
                     ['x-sil+x/調:x<x>x/詞:x!x@x/句:x^x_x',
                      'sil-g+ua/調:x<1>2/詞:0!1@1/句:0^3_3',
                      'g-ua+ʔ/調:x<1>2/詞:0!1@1/句:0^3_3',
    ...
    def test_臺灣客家話拼音完整標仔(self):
        句物件 = 拆文分析器.產生對齊句(self.客家話, self.客家話)
        音值句物件 = 轉物件音家私.轉音(臺灣客家話拼音, 句物件, 函式='通用音值')
        完整標仔 = 語音標仔轉換.物件轉完整合成標仔(音值句物件)
        self.檢驗標仔有對無(完整標仔,
                     ['x-sil+x/調:x<x>x/詞:x!x@x/句:x^x_x',
    ...
    def test_章物件轉合成標仔(self, 句物件轉標仔mock):
        ...
        句物件轉標仔mock.assert_has_calls([
            call(拆文分析器.建立句物件('食飽未？'), False),
            call(拆文分析器.建立句物件('食飽矣！'), False),
        ])
