...
class HTK辨識整合試驗(TestCase):
    ...
    def setUpClass(cls):
        ...
        模型 = 辨識模型()
        cls.特徵檔, cls.音節檔, cls.聲韻檔 = 模型.處理試驗語料(
            cls.音檔目錄,
            cls.試驗語料暫存目錄,
            cls.標仔目錄,
            cls.音節聲韻對照檔
    ...
    def test_原本標音模型訓練(self):
        ...
        self.檢查資料夾有辨識出來的檔案(
            self.模型.對齊聲韻(原本聲韻類檔, 原本模型檔, self.聲韻檔, self.特徵檔, 原本目錄)
        )
        self.檢查資料夾有辨識出來的檔案(
            self.模型.對齊音節(
    ...
    def test_拄好短恬模型訓練(self):
        ...
        self.檢查資料夾有辨識出來的檔案(
            self.模型.對齊聲韻(加短恬聲韻類檔, 加短恬模型檔, self.聲韻檔, self.特徵檔, 加短恬目錄)
        )
        self.檢查資料夾有辨識出來的檔案(
            self.模型.對齊音節(
    ...
    def test_三連音模型訓練(self):
        ...
        self.檢查資料夾有辨識出來的檔案(
            self.模型.對齊聲韻(三連音聲韻類檔, 三連音模型檔, self.聲韻檔, self.特徵檔, 三連音目錄)
        )
        self.檢查資料夾有辨識出來的檔案(
            self.模型.對齊音節(
