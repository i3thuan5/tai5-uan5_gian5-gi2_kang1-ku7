...
class 辭典揣詞試驗(TestCase):
	...
	def test_揣詞結果型態(self):
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.分析器.產生對齊字('我', 'gua2'))
		self.assertIsInstance(揣詞結果, 句)
		self.檢查分數詞數(分數, 詞數, 0.0, 1)
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.我對齊詞)
		self.assertIsInstance(揣詞結果, 句)
		self.檢查分數詞數(分數, 詞數, 0.0, 1)
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.我對齊組)
		self.assertIsInstance(揣詞結果, 句)
		self.檢查分數詞數(分數, 詞數, 0.0, 1)
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.我對齊集)
		self.assertIsInstance(揣詞結果, 句)
		self.檢查分數詞數(分數, 詞數, 0.0, 1)
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.句物件)
		self.assertIsInstance(揣詞結果, 句)
		self.檢查分數詞數(分數, 詞數, 0.0, 8)
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, 章([self.句物件]))
...
# 		raise RuntimeError('這个無穩定')
	...
	def test_集無使有多組(self):
		...
		self.assertRaises(解析錯誤,
			self.揣詞.揣詞, self.字典, self.文白我句物件)
	...
	def test_章揣詞(self):
		...
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, 章物件)
		self.assertEqual(揣詞結果, 結果章)
		self.檢查分數詞數(分數, 詞數, 0, 12)
		
	def test_空白組揣詞(self):
		self.字典.加詞(self.我對齊詞)
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.分析器.建立組物件(''))
	...
	def test_空白集揣詞(self):
		self.字典.加詞(self.我對齊詞)
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.分析器.建立集物件(''))
		self.assertEqual(揣詞結果, 句())
		self.assertEqual(分數, 0)
		self.assertEqual(詞數, 0)
	...
	def test_空白句揣詞(self):
		self.字典.加詞(self.我對齊詞)
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.分析器.建立句物件(''))
		self.assertEqual(揣詞結果, 句())
		self.assertEqual(分數, 0)
		self.assertEqual(詞數, 0)
	...
	def test_空白章揣詞(self):
		self.字典.加詞(self.我對齊詞)
		揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.分析器.建立章物件(''))
	...
	def test_詞屬性愛留咧(self):
		...
		self.驚對齊詞.屬性 = '袂使無去'
		self.字典.加詞(self.驚對齊詞)
		句物件, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.對齊句)
	...
	def test_章揣詞詞屬性愛留咧(self):
		...
		章物件 = self.分析器.產生對齊章(
			'我有一張椅仔！！', 'gua2 u7 tsit8-tiunn1 i2-a2!!')
		斷好句物件, 分數, 詞數 = self.揣詞.揣詞(self.字典, 章物件)
	...
	def test_加數字但是無揣詞(self):
		...
		題目 = '我1231我2+2'
		答案 = '我你我你你你'
		結果, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.分析器.建立句物件(答案))
		self.檢查分數詞數(分數, 詞數, 0, 6)
		...
		揣詞句物件, 分數, 詞數 = self.揣詞.揣詞(self.字典, self.分析器.建立句物件(題目))
