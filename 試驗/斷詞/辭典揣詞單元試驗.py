...
class 辭典揣詞單元試驗():
    ...
    def test_揣詞結果型態(self):
        揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.辭典, 拆文分析器.產生對齊字('我', 'gua2'))
...
# 		raise RuntimeError('這个無穩定')
    ...
    def test_愛選長詞(self):
        一對齊詞 = 拆文分析器.產生對齊詞('一', 'tsit8')
        張對齊詞 = 拆文分析器.產生對齊詞('張', 'tiunn1')
        self.辭典.加詞(一對齊詞)
        self.辭典.加詞(張對齊詞)
        self.辭典.加詞(self.一張對齊詞)
        揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.辭典, self.一張對齊詞)
        揣詞答案 = 拆文分析器.產生對齊句('一張', 'tsit8-tiunn1')
        self.assertEqual(揣詞結果, 揣詞答案)
        self.檢查分數詞數(分數, 詞數, 0, 1)

    def test_空白組揣詞(self):
        self.辭典.加詞(self.我對齊詞)
        揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.辭典, 拆文分析器.建立組物件(''))
    ...
    def test_空白集揣詞(self):
        self.辭典.加詞(self.我對齊詞)
        揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.辭典, 拆文分析器.建立集物件(''))
        self.assertEqual(揣詞結果, 句())
        self.assertEqual(分數, 0)
        self.assertEqual(詞數, 0)
    ...
    def test_空白句揣詞(self):
        self.辭典.加詞(self.我對齊詞)
        揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.辭典, 拆文分析器.建立句物件(''))
        self.assertEqual(揣詞結果, 句())
        self.assertEqual(分數, 0)
        self.assertEqual(詞數, 0)
    ...
    def test_空白章揣詞(self):
        self.辭典.加詞(self.我對齊詞)
        揣詞結果, 分數, 詞數 = self.揣詞.揣詞(self.辭典, 拆文分析器.建立章物件(''))
    ...
    def test_加數字但是無揣詞(self):
        ...
        題目 = '我1231我2+2'
        答案 = '我你我你你你'
        結果, 分數, 詞數 = self.揣詞.揣詞(self.辭典, 拆文分析器.建立句物件(答案))
        self.檢查分數詞數(分數, 詞數, 0, 6)
        ...
        揣詞句物件, 分數, 詞數 = self.揣詞.揣詞(self.辭典, 拆文分析器.建立句物件(題目))
