...
class 斷詞斷字翻譯試驗(TestCase):
	def setUp(self):
		...
		self.閩南語句物件 = self.分析器.建立句物件('')
		self.閩南語句物件.內底集 = [閩南語集物件]
	...
	def test_干焦用著斷詞用戶端的翻譯(self, 翻譯句物件mock):
		...
		翻譯句物件mock.assert_called_once_with(self.華語句物件)
	def test_翻譯結果是句物件(self):
		self.xmlrpcMock.return_value.translate.return_value = self.斷詞孤字未知詞譯孤字
	...
	def test_斷詞孤字未知詞譯雙字(self):
		self.xmlrpcMock.return_value.translate.return_value = self.斷詞孤字未知詞譯雙字
		結果句物件, _, _ = self.用戶端.翻譯(self.華語句物件)
		self.assertEqual(結果句物件, self.閩南語句物件)
	def test_斷詞雙字未知詞譯雙字(self):
		self.xmlrpcMock.return_value.translate.return_value = self.斷詞雙字未知詞譯雙字
		結果句物件, _, _ = self.用戶端.翻譯(self.華語句物件)
		self.assertEqual(結果句物件, self.閩南語句物件)
	@patch('臺灣言語工具.翻譯.摩西工具.摩西用戶端.摩西用戶端._翻譯句物件')
