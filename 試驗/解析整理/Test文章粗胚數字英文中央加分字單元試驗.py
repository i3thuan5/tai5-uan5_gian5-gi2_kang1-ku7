...
class 文章粗胚數字英文中央加分字單元試驗(unittest.TestCase):
    ...
    def test_全加對齊組大寫音標愛隔開(self):
        原本 = 'Sui2sui2 是 咱 的 好 厝邊'
        答案 = 'Sui2-sui2 是 咱 的 好 厝邊'
        結果 = 文章粗胚.數字英文中央全加分字符號(原本)
        self.assertEqual(結果, 答案)

    def test_全加對齊組小寫音標愛隔開(self):
        原本 = 'sui2sui2 是 咱 的 好 厝邊'
        答案 = 'sui2-sui2 是 咱 的 好 厝邊'
        結果 = 文章粗胚.數字英文中央全加分字符號(原本)
        self.assertEqual(結果, 答案)

    def test_全加對齊組三連媠愛隔開(self):
        原本 = 'sui2sui2sui2 是 咱 的 好 厝邊'
        答案 = 'sui2-sui2-sui2 是 咱 的 好 厝邊'
        結果 = 文章粗胚.數字英文中央全加分字符號(原本)
        self.assertEqual(結果, 答案)

    def test_全加對齊輕聲(self):
        原本 = 'li2 ho2-0bo5'
        答案 = 'li2 ho2-0bo5'
        結果 = 文章粗胚.數字英文中央全加分字符號(原本)
        self.assertEqual(結果, 答案)
    ...
    def test_看情形大寫專有符號袂使拆開(self):
        原本 = 'H1N1 新型 流感 包含 四種 病毒'
        結果 = 文章粗胚.數字英文中央看情形加分字符號(原本, {'H1N1'})
        self.assertEqual(結果, 原本)

    def test_看情形小寫專有符號袂使拆開(self):
        原本 = 'g0v 是 咱 的 好 厝邊'
        結果 = 文章粗胚.數字英文中央看情形加分字符號(原本, {'g0v'})
        self.assertEqual(結果, 原本)

    def test_看情形對齊組大寫音標袂使拆開(self):
        原本 = 'Sui2sui2 是 咱 的 好 厝邊'
        結果 = 文章粗胚.數字英文中央看情形加分字符號(原本, {'Sui2sui2'})
        self.assertEqual(結果, 原本)
