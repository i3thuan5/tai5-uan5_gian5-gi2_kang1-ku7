...
class 拆文分析器轉做單元試驗(unittest.TestCase):
    ...
    def test_轉做字有兩字(self):
        分詞 = '兩｜nng7 个｜e5'
        self.assertRaises(解析錯誤, 拆文分析器.轉做字物件, 分詞)
    ...
    def test_轉做詞無對齊(self):
        分詞 = '美-麗｜sui2'
        self.assertRaises(解析錯誤, 拆文分析器.轉做詞物件, 分詞)
    ...
    def test_轉做組集句章孤字(self):
        ...
        self.assertEqual(組物件.內底詞[0], 拆文分析器.轉做詞物件('𪜶｜in1'))
        集物件 = 拆文分析器.轉做集物件(分詞)
    ...
    def test_轉做組集句章濟字(self):
        ...
        self.assertEqual(組物件.內底詞[0], 拆文分析器.轉做詞物件('𪜶｜in1'))
        self.assertEqual(組物件.內底詞[1], 拆文分析器.轉做詞物件('兩｜nng7'))
        self.assertEqual(組物件.內底詞[2], 拆文分析器.轉做詞物件('个｜e5'))
        self.assertEqual(組物件.內底詞[3], 拆文分析器.轉做詞物件('生-做｜senn1-tso3'))
        self.assertEqual(
            組物件.內底詞[4], 拆文分析器.轉做詞物件('一-模-一-樣｜it4-boo5-it4-iunn7'))
        self.assertEqual(組物件.內底詞[5], 拆文分析器.轉做詞物件('。｜.'))
        集物件 = 拆文分析器.轉做集物件(分詞)
        self.assertEqual(len(集物件.內底組), 1)
        self.assertEqual(集物件.內底組[0], 組物件)
        句物件 = 拆文分析器.轉做句物件(分詞)
    ...
    def test_轉做組集句章濟字佮有連字(self):
        ...
        組物件 = 拆文分析器.轉做組物件(分詞)
        self.assertEqual(len(組物件.內底詞), 6)
        self.assertEqual(組物件.內底詞[0], 拆文分析器.轉做詞物件('兩｜nng7'))
        self.assertEqual(組物件.內底詞[1], 拆文分析器.轉做詞物件('个｜e5'))
        self.assertEqual(組物件.內底詞[2], 拆文分析器.轉做詞物件('-｜-'))
        self.assertEqual(組物件.內底詞[3], 拆文分析器.轉做詞物件('-｜-'))
        self.assertEqual(
            組物件.內底詞[4], 拆文分析器.轉做詞物件('一-模-一-樣｜it4-boo5-it4-iunn7'))
        self.assertEqual(組物件.內底詞[5], 拆文分析器.轉做詞物件('。｜.'))
        集物件 = 拆文分析器.轉做集物件(分詞)
    ...
    def test_轉做章濟句(self):
        ...
        self.assertEqual(章物件.內底句[0], 拆文分析器.轉做句物件(分詞一))
        self.assertEqual(章物件.內底句[1], 拆文分析器.轉做句物件(分詞二))
    ...
    def test_烏白傳參數(self):
        self.assertRaises(型態錯誤, 拆文分析器.轉做字物件, None)
        self.assertRaises(型態錯誤, 拆文分析器.轉做詞物件, 333)
        self.assertRaises(型態錯誤, 拆文分析器.轉做組物件, [])
        self.assertRaises(型態錯誤, 拆文分析器.轉做集物件, None)
        self.assertRaises(型態錯誤, 拆文分析器.轉做句物件, 14325)
        self.assertRaises(型態錯誤, 拆文分析器.轉做章物件, ('None',))
    ...
    def test_接受詞內底無一定全部攏愛有音(self):
        ...
        答案詞陣列 = [
            ...
            山詞物件, 鄉所詞物件, 拆文分析器.產生對齊詞('猴山', 'kau5-san1'),
        ]
        self.assertEqual(組物件.內底詞, 答案詞陣列)
