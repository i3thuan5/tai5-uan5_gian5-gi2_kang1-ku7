...
class 拆文分析器建立單元試驗(unittest.TestCase):
    ...
    def test__拆章做句(self):
        self.assertEqual(拆文分析器._拆章做句('我腹肚枵，欲來去食飯。'), ['我腹肚枵，', '欲來去食飯。'])
        self.assertEqual(
            拆文分析器._拆章做句('伊講：我腹肚枵，欲來去食飯。'), ['伊講：我腹肚枵，', '欲來去食飯。'])
        self.assertEqual(
            拆文分析器._拆章做句('伊講:我腹肚枵，欲來去食飯。'), ['伊講:我腹肚枵，', '欲來去食飯。'])
        self.assertEqual(拆文分析器._拆章做句('這馬分數1:2，誠緊張。'), ['這馬分數1:2，', '誠緊張。'])
        self.assertEqual(拆文分析器._拆章做句('今日8/30。'), ['今日8/30。'])
        self.assertEqual(拆文分析器._拆章做句('啥物！！？你轉去矣？'), ['啥物！！？', '你轉去矣？'])
        self.assertEqual(拆文分析器._拆章做句('！！？你轉去？矣'), ['！！？', '你轉去？', '矣'])
        self.assertEqual(拆文分析器._拆章做句('你！！？轉去？矣'), ['你！！？', '轉去？', '矣'])
        self.assertEqual(拆文分析器._拆章做句('！你！？轉去？矣'), ['！', '你！？', '轉去？', '矣'])
        self.assertEqual(
            拆文分析器._拆章做句('！你！？轉去？矣？？'), ['！', '你！？', '轉去？', '矣？？'])
        self.assertEqual(拆文分析器._拆章做句('！！。。，。你好？'), ['！！。。，。', '你好？'])
        self.assertEqual(
            拆文分析器._拆章做句('!!..,.li2 ho2?'), ['!!..,.', 'li2 ho2?'])
        self.assertEqual(
            拆文分析器._拆章做句('!!..,. li2 ho2?'), ['!!..,. ', 'li2 ho2?'])
    ...
    def test__拆章做句配分詞符號(self):
        ...
        self.assertEqual(拆文分析器._拆章做句(原來), 答案)
        self.assertEqual(拆文分析器._拆章做句(加空白), 空白答案)

    # 	_拆章做句莫加空白，保留原本的，若有空白佇中央，有標點就加
    # 	按呢才有法度處理其他文本
