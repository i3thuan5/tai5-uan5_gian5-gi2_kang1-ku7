...
class 拆文分析器建立單元試驗(unittest.TestCase):
    ...
    def test_無空白分開的閩南語音標(self):
        ...
        無空白 = 有空白.replace(' ', '')
        self.assertNotEqual(拆文分析器.建立組物件(有空白),
                            拆文分析器.建立組物件(無空白))
    ...
    def test_拆句做字(self):
        self.assertEqual(拆文分析器.拆句做字('腹肚枵'), ['腹', '肚', '枵'])
        self.assertRaises(型態錯誤, 拆文分析器.拆句做字, None)
    ...
    def test_拆句做字無愛空白(self):
        self.assertEqual(拆文分析器.拆句做字('腹 肚枵矣'), ['腹', '肚', '枵', '矣'])
        self.assertEqual(拆文分析器.拆句做字('  腹 肚枵矣  '), ['腹', '肚', '枵', '矣'])

    def test_拆句做字摻組字式(self):
        self.assertEqual(拆文分析器.拆句做字('⿰因腹肚枵'), ['⿰因', '腹', '肚', '枵'])
        self.assertEqual(
            拆文分析器.拆句做字('你同⿰厓去睡目。'), ['你', '同', '⿰厓', '去', '睡', '目', '。'])
        self.assertEqual(拆文分析器.拆句做字('⿰22腹肚枵'), ['⿰2', '2', '腹', '肚', '枵'])
        self.assertRaises(解析錯誤, 拆文分析器.拆句做字, '腹肚枵⿰')
        self.assertRaises(解析錯誤, 拆文分析器.拆句做字, '腹肚枵⿰⿰')
        self.assertEqual(拆文分析器.拆句做字('腹肚枵⿰⿰因'), ['腹', '肚', '枵', '⿰⿰因'])

    def test_拆句做字摻漢羅佮數字(self):
        self.assertEqual(拆文分析器.拆句做字('腹肚枵ah'), ['腹', '肚', '枵', 'ah'])
        self.assertEqual(
            拆文分析器.拆句做字('我e腹肚枵ah'), ['我', 'e', '腹', '肚', '枵', 'ah'])
        self.assertEqual(
            拆文分析器.拆句做字('我ê腹肚枵ah'), ['我', 'ê', '腹', '肚', '枵', 'ah'])
        self.assertEqual(
            拆文分析器.拆句做字('我ê pak tóo枵ah'), ['我', 'ê', 'pak', 'tóo', '枵', 'ah'])
        self.assertEqual(
            拆文分析器.拆句做字('我ê pak-tóo枵ah'), ['我', 'ê', 'pak', 'tóo', '枵', 'ah'])
        self.assertEqual(
            拆文分析器.拆句做字('我ê pak - tóo枵ah'), ['我', 'ê', 'pak', '-', 'tóo', '枵', 'ah'])
        ...
        self.assertEqual(
            拆文分析器.拆句做字('我ê pak-tóo枵ah,.'), ['我', 'ê', 'pak', 'tóo', '枵', 'ah', ',', '.'])
        self.assertEqual(拆文分析器.拆句做字('我有100箍'), ['我', '有', '100', '箍', ])
        self.assertEqual(
            拆文分析器.拆句做字('這馬時間12:20，'), ['這', '馬', '時', '間', '12', ':', '20', '，'])
        self.assertEqual(
            拆文分析器.拆句做字('物件tsin1 ho2食。'), ['物', '件', 'tsin1', 'ho2', '食', '。'])

    def test_拆句做巢狀詞摻漢羅佮數字(self):
        self.assertEqual(
            拆文分析器.拆句做巢狀詞('腹肚枵ah'), [['腹'], ['肚'], ['枵'], ['ah']])
        self.assertEqual(
            拆文分析器.拆句做巢狀詞('我ê腹肚枵ah'), [['我'], ['ê'], ['腹'], ['肚'], ['枵'], ['ah']])
        self.assertEqual(拆文分析器.拆句做巢狀詞('我ê pak tóo枵ah'),
                         [['我'], ['ê'], ['pak'], ['tóo'], ['枵'], ['ah']])
        self.assertEqual(
            拆文分析器.拆句做巢狀詞('我ê pak-tóo枵ah'), [['我'], ['ê'], ['pak', 'tóo'], ['枵'], ['ah']])
        self.assertEqual(拆文分析器.拆句做巢狀詞(
    ...
    def test_拆句做巢狀詞摻組字式(self):
        ...
        斷詞後巢狀陣列 = [['⿰--', '⿰--'], ['-'], ['⿱--'],
                   [','], ['⿰-,'], ['⿱⿰-,-', '⿱--']]
        self.assertEqual(拆文分析器.拆句做巢狀詞(原本語句), 斷詞後巢狀陣列)

    def test_拆章做句(self):
        self.assertEqual(拆文分析器.拆章做句('我腹肚枵，欲來去食飯。'), ['我腹肚枵，', '欲來去食飯。'])
        self.assertEqual(
            拆文分析器.拆章做句('伊講：我腹肚枵，欲來去食飯。'), ['伊講：我腹肚枵，', '欲來去食飯。'])
        self.assertEqual(
            拆文分析器.拆章做句('伊講:我腹肚枵，欲來去食飯。'), ['伊講:我腹肚枵，', '欲來去食飯。'])
        self.assertEqual(拆文分析器.拆章做句('這馬分數1:2，誠緊張。'), ['這馬分數1:2，', '誠緊張。'])
        self.assertEqual(拆文分析器.拆章做句('今日8/30。'), ['今日8/30。'])
        self.assertEqual(拆文分析器.拆章做句('啥物！！？你轉去矣？'), ['啥物！！？', '你轉去矣？'])
        self.assertEqual(拆文分析器.拆章做句('！！？你轉去？矣'), ['！！？', '你轉去？', '矣'])
        self.assertEqual(拆文分析器.拆章做句('你！！？轉去？矣'), ['你！！？', '轉去？', '矣'])
        self.assertEqual(拆文分析器.拆章做句('！你！？轉去？矣'), ['！', '你！？', '轉去？', '矣'])
        self.assertEqual(
            拆文分析器.拆章做句('！你！？轉去？矣？？'), ['！', '你！？', '轉去？', '矣？？'])
        self.assertEqual(拆文分析器.拆章做句('！！。。，。你好？'), ['！！。。，。', '你好？'])
        self.assertEqual(
            拆文分析器.拆章做句('!!..,.li2 ho2?'), ['!!..,.', 'li2 ho2?'])
        self.assertEqual(
            拆文分析器.拆章做句('!!..,. li2 ho2?'), ['!!..,. ', 'li2 ho2?'])
    ...
    def test_拆章做句配分詞符號(self):
        ...
        答案 = ['!!..,.', 'li2-ho2?']
        空白答案 = [' ! ! . . , . ', 'li2-ho2 ? ']
        self.assertEqual(文章粗胚.符號邊仔加空白(原來), 加空白)
        self.assertEqual(拆文分析器.拆章做句(原來), 答案)
        self.assertEqual(拆文分析器.拆章做句(加空白), 空白答案)
    ...
    def test_拆章做句有分字符號配分詞符號(self):
        ...
        self.assertEqual(文章粗胚.建立物件語句前處理減號(臺灣閩南語羅馬字拼音, 原來), 處理減號)
        self.assertEqual(文章粗胚.符號邊仔加空白(處理減號), 加空白)
        self.assertEqual(拆文分析器.拆章做句(加空白), 空白答案)
