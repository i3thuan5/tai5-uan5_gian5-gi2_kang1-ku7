...
class 程式腳本單元試驗(TestCase):
    ...
    def test_讀寫一般檔案(self):
        檔名 = os.path.join(self.這馬目錄, '文件.txt')
        程式腳本._陣列寫入檔案(檔名, ['臺灣', '建國'])
        self.assertEqual(程式腳本._讀檔案(檔名), ['臺灣', '建國'])
        os.remove(檔名)
    ...
    def test_檔案合做一个(self):
        ...
        程式腳本._字串寫入檔案(臺灣檔名, '臺灣')
        建國檔名 = os.path.join(self.這馬目錄, '建國.txt.gz')
        程式腳本._字串寫入檔案(建國檔名, '建國')
        上尾平行語料檔名 = os.path.join(self.這馬目錄, '文件.txt')
        程式腳本._檔案合做一个(上尾平行語料檔名, [臺灣檔名, 建國檔名], 無編碼器())
        self.assertEqual(程式腳本._讀檔案(上尾平行語料檔名), ['臺灣', '建國'])
        os.remove(臺灣檔名)
        os.remove(建國檔名)
        os.remove(上尾平行語料檔名)
